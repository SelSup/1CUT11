...
Процедура ВыполнитьКоманду(ИдентификаторКоманды) Экспорт
	...
	Если ИдентификаторКоманды = "ОбновитьЗаказыСSelSup" Тогда  
		...
		//TODO - в отдельный метод и отбор по организации?
		Запрос = Новый Запрос(
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	SS_КлючиAPI.КлючAPI КАК КлючAPI
		|ИЗ
		...
		ТаблицаОрганизацийДляАвтоОбмена = Запрос.Выполнить().Выгрузить(); 
		
		//TODO
		Если ТаблицаОрганизацийДляАвтоОбмена.Количество() > 0 Тогда  
			ЗаполнитьЗначенияСвойств(ЭтотОбъект, ТаблицаОрганизацийДляАвтоОбмена[0]);
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(Организация) Тогда  
			...
			|ГДЕ
			|	SS_РасширеннаяНастройкаЗаказов.Организация = &Организация");  
			
			Запрос.УстановитьПараметр("Организация", Организация);
			ТаблицаНастройкаЗаказов = Запрос.Выполнить().Выгрузить(); 
			
			Если ТаблицаНастройкаЗаказов.Количество() > 1 Тогда 
				ИспользоватьРасширеннуюНастройкуЗаказов = Истина;
				РасширеннаяНастройкаЗаказов.Очистить();  
				Для каждого СтрокаНастройки Из ТаблицаНастройкаЗаказов Цикл
				...
				Если ТаблицаНастройкаЗаказов.Количество() = 1 Тогда 
					...
					РасширеннаяНастройкаЗаказов.Очистить();
					ЗаполнитьЗначенияСвойств(ЭтотОбъект, СтрокаНастройки[0]); 
					ЭтотОбъект.ВыбКонтрагент = СтрокаНастройки[0].Контрагент;
					ЭтотОбъект.ВыбСоглашение= СтрокаНастройки[0].Соглашение;
					ЭтотОбъект.ВыбДоговор = СтрокаНастройки[0].Договор;
				КонецЕсли;
		...
		Если ТаблицаКлючейAPI.Количество() > 0 Тогда  
			ApiKey = ТаблицаКлючейAPI[0].КлючAPI;
			ЗаписьЖурналаРегистрации("ИнтеграцияСSelSup.ОбновитьЗаказыСSelSup", УровеньЖурналаРегистрации.Информация, , , "Ключ API получен.");
			ЗаполнитьНайстройкиПоУмолчанию();
			СформироватьОтправления("FBS", Истина);  
			СформироватьЗаказыПокупателей();
		Иначе
			ЗаписьЖурналаРегистрации("ИнтеграцияСSelSup.ОбновитьЗаказыСSelSup", УровеньЖурналаРегистрации.Ошибка, , , "Ошибка при получении ключа API");	
		КонецЕсли;
